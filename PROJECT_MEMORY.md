...
- [ ] **Milestone 18 (Production Assurance & Red-Team Evidence — release blockers documented):** Production modules are exercised against live local PostgreSQL/pgvector and Redis; AC-5 probes locked HTTP endpoints (`/api/run-skill` and `/api/ask-vault`), AC-8 invokes `/api/sso/deprovision`, and AC-9 attacks authenticated Streamable HTTP MCP. The external KMS, Claude, and embedding endpoints used by the hermetic acceptance suite are explicitly classified as contract-test doubles. Azure Key Vault wrapping support is implemented. Epic 8 reconciliation added ADR-019 and the data dictionary. Production approval remains blocked until genuine cloud-KMS and hosted-LLM smoke-test evidence is attached, `timeline_entries.entry_text` is migrated from plaintext storage, and the user explicitly approves Gate 7/8.
...
| **Searchable Encryption** | AES-256-GCM + tsvector + Dense Vector | **ADR-017 Decided:** Open vault searchable boundaries with encrypted chunks, lexical tsvectors, and 1536-dim pgvector embeddings; locked vaults have zero chunks/vectors/tsvectors. |
| **Cross-Vault Link Policy** | Strict Vault-Local Scoping + Ghost Links | **ADR-018 Decided:** Resolved links strictly intra-vault (`from_page.vault_id == to_page.vault_id`). Missing/cross-vault links stored as ghost links (`to_page_id = null, resolved = false`). Zero Markdown body mutation. |
...
- [x] **2026-09-18:** Completed Epic 6 (Vault Lifecycle, Page Movement, and Cross-Vault Link Policy & Graph Integrity):
  - **Chunk 6.1 (Transactional Vault Creation):** Server-side validation, atomic creation of vault, owner share, and audit event in PostgreSQL, independent KMS DEK wrapping, and anti-disclosure client responses (6/6 vault-core, 3/3 api-server tests pass).
  - **Chunk 6.2 (Atomic Page Movement):** Shared page-mutation locking contract (`lockPageForMutation`), post-lock DEK and concurrency resolution, multi-version/multi-chunk re-encryption across distinct KMS keys, mode-aware index segregation (zero-read locked purge vs. open indexing), post-mutation rollback evidence, and deterministic live PostgreSQL concurrency races with controllable barriers (12/12 vault-move tests, 4/4 api-server tests pass).
  - **Chunk 6.3 (Cross-Vault Link Policy & Graph Integrity):** Authored `ADR-018` establishing strict intra-vault link scoping (`from_page.vault_id == to_page.vault_id`) and ghost link semantics (`to_page_id = null, resolved = false`). Refactored `LinkGraphIndexer` public methods (`updateLinksForPage`, `resolveIncomingGhostLinks`) to derive authoritative vault ID under row lock and transaction, eliminating caller vault ID bypasses.
    - Added real tracked forward database migration (`0001_add_link_graph_invariants.sql`) and journal entry, establishing a single canonical migration and DDL source.
    - Enforced exact logical equivalence CHECK constraint `links_resolved_has_target` (`resolved = (to_page_id IS NOT NULL)`), forbidding invalid ghost states with non-null targets.
    - Installed deferred constraint triggers (`trg_links_enforce_invariants`, `trg_pages_enforce_link_invariants`, `trg_vaults_enforce_link_invariants`) and automatic deletion-to-ghost trigger (`trg_links_before_update`).
    - Subordinated `ensureLinkGraphInvariants` to the canonical migration runner `runMigrations()`.
    - Hardened production Docker and deployment database provisioning: added `migration` service to `docker-compose.yml`, configured `depends_on: migration: condition: service_completed_successfully` for `api-server`, `skill-runner`, and `mcp-gateway`, and added startup migration checks in `apps/api-server/src/server.ts`.
    - Added dedicated database provisioning test suite (`services/vault-core/test/database-provisioning.test.js`) verifying fresh DB provisioning, upgraded DB provisioning, constraint validation (`convalidated = true`), trigger enablement (`tgenabled = 'O'`), and deletion ghosting.
    - Hardened REST and MCP production paths with live Express and Streamable HTTP tests (20/20 cross-vault-links tests, 3/3 database-provisioning tests, 22/22 api-server tests, 40/40 mcp-gateway tests pass). Full uncached monorepo suite: 12/12 tasks pass.
  - **Review Gate 6:** Explicitly approved by the user on 2026-09-18.
- [x] **2026-09-18:** Implemented the Review Gate 3 production-path helper dispatch remediation:
  - Added versioned encrypted skill packages (`tkxel-skill-package/v1`) containing `SKILL.md`, validated `tool.json`, and package-relative helper files, while preserving legacy prompt-only skill compatibility.
  - The real MCP Streamable HTTP `run_skill` path now crosses the signed skill-runner HTTP boundary, authorizes against PostgreSQL, unwraps the vault DEK, parses and validates the encrypted helper manifest, and dispatches helpers through `SandboxRunner`.
  - Helper source and user input are streamed over stdin, never written to disk or placed in process arguments. Production execution remains container-only, default-deny network, non-root, read-only, resource limited, and fail-closed without registered `runsc`.
  - Focused verification: 14 manifest/orchestrator/sandbox tests, 13 independent runner HTTP test events, and 5 MCP integration test events pass. Final complete skill-runner suite: 93 dot-reporter test events, 0 failures. MCP gateway build and complete suite pass with exit status 0.
  - Updated Gate 3 Linux acceptance phase 5 to exercise the real MCP/HTTP helper path under live `runsc` instead of invoking `SandboxRunner` directly.
- **Review Gate 3:** APPROVED BY USER on 2026-09-18. Executed `scripts/verify-gate3-linux-acceptance.sh` on WSL2 Ubuntu 24.04 LTS (kernel 6.18.33.2-microsoft-standard-WSL2) with official Google gVisor `runsc` (release-20260914.0). All 25/25 checks passed with 0 failures: verified live container gVisor kernel virtualization, production stack health, port 3003 host isolation and internal Docker bridge connectivity, container hardening (UID 10001, read-only rootfs, `no-new-privileges:true`, cap-drop ALL), live encrypted helper execution through MCP Streamable HTTP with `runsc`, deep sandbox security (noexec tmpfs, default-deny network egress, zero host mounts, secret isolation, PID limits, 64KB output truncation, 4s execution timeout), HMAC service token contract with Redis replay prevention (`AUTH_REPLAY_OK`), and clean database audit canary scan. The user explicitly accepted WSL2 evidence instead of requiring a separate Linux VM/CI runner and authorized Epic 7. Evidence log recorded at `docs/acceptance-evidence/GATE-3-LINUX-RUNSC-EVIDENCE.log`.
- [x] **2026-09-18:** Completed Epic 7 Production-Assurance Remediation with exact security corrections:
  - **Fail-Closed MCP Gateway Authorization:** Removed `vlt_default` fallback in `services/mcp-gateway/src/server.ts`. Authorization lookup failure or missing memberships strictly returns `[]` (empty tool palette). Added regression test in `e2e-gateway-server.test.js`.
  - **Fail-Closed SSO Deprovisioning Webhook:** Removed hard-coded webhook secret fallback in `services/mcp-gateway/src/server.ts`. Endpoint fails closed with HTTP 500 when no secret is injected or configured in environment. Added regression test in `e2e-gateway-server.test.js`.
  - **Production KMS Invariant Preserved:** Strictly disallowed `KMS_PROVIDER=http` for production deployments. `HttpKmsProvider` is acceptance-only and restricted to loopback contract-test-double endpoints when `ACCEPTANCE_TESTING=true`. Production supports AWS KMS (`aws`) and Azure Key Vault (`azure`); Azure uses Key Vault `wrapkey`/`unwrapkey` calls and requires workload-identity token configuration.
  - **Skill Runner Readiness Policy Alignment:** Corrected `GET /ready` and `validateProductionEnvironment()` in `skill-runner` to enforce the permitted provider policy (`aws` with `AWS_KMS_KEY_ID`, `azure` with `AZURE_KEY_VAULT_URL`, or `http` strictly when `ACCEPTANCE_TESTING=true`).
  - **Accurate Wire Protocol Contract-Test Doubles:** Local HTTP KMS, Claude Messages, and OpenAI Embeddings servers are **contract-test doubles** (verifying HTTP wire protocols, headers, serialization, and error schemas hermetically). They cannot support a production-approved release claim; the release guard rejects one that relies on them.
  - **Release Claims Guard Hardening:** Enforces `Passed checks: 25` AND `Failed checks: 0` for `.log` verification, and prevents manifest status inconsistency while compliance report is in remediation. Verified `release-evidence.json` status set to `in-remediation`.
  - **Verification Evidence:**
    - Acceptance Suite (`node --test scripts/verify-acceptance-criteria.js`): 10/10 passing (0 failed, duration ~4.4s).
    - Release Claims Guard Tests (`node --test scripts/verify-release-claims.test.mjs`): 9/9 passing (0 failed).
    - Release Claims Guard (`node scripts/verify-release-claims.mjs`): Pass (exit 0).
    - Package Suites: `@tkxel-vault/vault-core` 139/139 passing; `@tkxel-vault/skill-runner` 93/93 passing; `@tkxel-vault/mcp-gateway` 45/45 passing.
  - **Review Gate 7 Status:** Implementation remediation is verified; Review Gate 7 is NOT marked approved. Genuine cloud-KMS and hosted-LLM smoke-test evidence remains required for production approval.
- [x] **2026-09-18:** Restored the web-app browser/server module boundary by removing the database-backed `LinkGraphIndexer` from the browser-facing vault-core markdown barrel. Added a regression test; 125/125 web-app tests pass, TypeScript passes, and the production bundle contains no Node `events`, `Buffer`, or PostgreSQL signatures.
